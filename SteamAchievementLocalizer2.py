...
from bin_utils import replace_all_columns_in_bin
...
def save_bin_unknow(self):
    datas = replace_all_columns_in_bin(self.headers, self.table, self.steam_folder, self.game_id())
    if datas:  # Перевір, чи не None
        ...
        QMessageBox.warning(self, "Помилка", "Не вдалося згенерувати дані для збереження")
